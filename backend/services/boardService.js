...
const boardParticipantIds = (board) => [
  board.ownerId,
  ...(board.membersId || []),
];

const notifyBoardParticipants = async ({
  board,
  fromId,
  type,
  title,
  ...
  const recipients = boardParticipantIds(board).filter(
    (userId) => !excludeActor || !isSameId(userId, fromId),
  );
...
const deleteBoardForUser = async (boardId, userId) => {
  ...
  // Notify board participants about the deletion
